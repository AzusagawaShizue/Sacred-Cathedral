  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(85vh-180px)]" style={{ background: 'linear-gradient(180deg, #FAF7F0 0%, #F2EDDF 100%)' }}>
          ...
          >
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#EEDCB3] to-[#E4D2A6] flex items-center justify-center flex-shrink-0 shadow-lg">
                <Star className="w-6 h-6 text-[#1E2A5A]" />
              </div>
              <div>
                <h4 className="text-[#1E2A5A] mb-2">祈祷机制</h4>
                <p className="text-sm text-[#1E2A5A]/70 leading-relaxed">
            ...
            {rarityData.map((tier, index) => (
              ...
              >
                ...
                <div className="p-5 glass-morphism">
                  <div className="mb-4">
                    <div className="text-xs text-[#1E2A5A]/60 mb-2 tracking-wide">包含物品</div>
                    <div className="flex flex-wrap gap-2">
                      {tier.items.map((item) => (
                        <motion.span
                          key={item}
                          whileHover={{ scale: 1.05 }}
                          className="px-3 py-2 glass-gold text-[#1E2A5A] text-sm rounded-xl shadow-md"
                        >
                          {item}
                        </motion.span>
                      ))}
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-[#1E2A5A]/10">
                    <span className="text-sm text-[#1E2A5A]/70">预估价值范围</span>
                    <span className="text-[#1E2A5A] font-mono">{tier.value}</span>
                  </div>
                </div>
              </motion.div>
            ))}
          ...
          >
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#295EE7] to-[#1E2A5A] flex items-center justify-center flex-shrink-0 shadow-lg">
                <TrendingUp className="w-6 h-6 text-white" />
              </div>
              <div>
                <h4 className="text-[#1E2A5A] mb-2">价值提示</h4>
                <p className="text-sm text-[#1E2A5A]/70 leading-relaxed">
        ...
        {/* Footer */}
        <div className="sticky bottom-0 p-4 glass-gold border-t border-[#EEDCB3]/30">
          <motion.button
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            onClick={onClose}
            className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#295EE7] to-[#1E2A5A] text-white shadow-xl relative overflow-hidden"
          >
            <motion.div
