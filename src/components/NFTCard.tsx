      ...
      >
        <div className={`glass-morphism rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 ${rarityStyle.borderGlow}`}
          ...
          <div className="p-4 glass-gold">
            <h3 className="text-[#1E2A5A] mb-1 truncate">{nft.name}</h3>
            <p className="text-xs text-[#1E2A5A]/60 mb-3 line-clamp-2 leading-relaxed">
              {nft.description}
            </p>
            ...
            {nft.price !== undefined && (
              <div className="flex items-center justify-between mb-3 p-3 rounded-xl glass-morphism shadow-inner">
                <span className="text-xs text-[#1E2A5A]/70">价格</span>
                <span className="text-[#1E2A5A] font-mono text-sm">{nft.price.toFixed(3)} ETH</span>
              </div>
            )}

            {/* Owner Info (Market Mode) */}
            {mode === 'buy' && nft.owner && (
              <div className="flex items-center justify-between mb-3 p-2 rounded-lg bg-[#1E2A5A]/5">
                <span className="text-xs text-[#1E2A5A]/60">持有者</span>
                <span className="text-xs text-[#1E2A5A] font-mono">{nft.owner}</span>
              </div>
            )}
            ...
            {mode === 'buy' ? (
              <motion.button
                ...
                className="w-full py-3 rounded-xl bg-gradient-to-r from-[#295EE7] to-[#1E2A5A] text-white shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center gap-2 relative overflow-hidden group"
              >
                <motion.div
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                {nft.listed ? (
                  <button className="py-3 rounded-xl glass-gold text-[#1E2A5A]/70 border border-[#EEDCB3] text-sm">
                    已上架
                  </button>
                ) : (
                  <motion.button
                    ...
                    className="py-3 rounded-xl bg-gradient-to-r from-[#EEDCB3] to-[#E4D2A6] text-[#1E2A5A] shadow-lg flex items-center justify-center gap-1 text-sm"
                  >
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-[#1E2A5A]">上架NFT</h3>
                <button
                  onClick={() => setShowListModal(false)}
                  className="w-9 h-9 rounded-lg glass-gold hover:bg-[#EEDCB3]/50 flex items-center justify-center transition-colors"
                >
                  <X className="w-5 h-5 text-[#1E2A5A]" />
                </button>
              </div>
              ...
              <div className="mb-5">
                <div className="text-center py-5 rounded-2xl glass-gold mb-5">
                  ...
                  <p className="text-[#1E2A5A]">{nft.name}</p>
                </div>

                <label className="block text-sm text-[#1E2A5A]/70 mb-2">设置价格 (ETH)</label>
                <input
                  type="number"
                  step="0.001"
                  min="0"
                  ...
                  placeholder="0.000"
                  className="w-full px-4 py-3 rounded-xl glass-morphism focus:outline-none focus:ring-2 focus:ring-[#295EE7] transition-all text-[#1E2A5A]"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={() => setShowListModal(false)}
                  className="py-3 rounded-xl glass-gold hover:bg-[#EEDCB3]/50 text-[#1E2A5A] transition-colors"
                >
                  取消
                </motion.button>
                <motion.button
                  ...
                  className="py-3 rounded-xl bg-gradient-to-r from-[#295EE7] to-[#1E2A5A] text-white shadow-lg relative overflow-hidden"
                >
                  <motion.div
                    className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent"
                    animate={{
